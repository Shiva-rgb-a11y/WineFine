...
def read_yml(path_to_yaml: str) -> dict:
    try:
        ...
        logging.error(f"Failed to read YAML file: {path_to_yaml} — {e}")
        raise
...
def create_directories(path):
    try:
        ...
        logging.error(f"Failed to create directories: {path} — {e}")
        raise e
...
def save_bin(path: str, data):
    try:
        ...
        logging.info(f"Binary data saved using joblib: {path}")
    except Exception as e:
        logging.error(f"Failed to save binary file: {path} — {e}")
        raise
...
def load_bin(path: str):
    try:
        ...
        logging.info(f"Binary data loaded using joblib: {path}")
        return data
    except Exception as e:
        logging.error(f"Failed to load binary file: {path} — {e}")
        raise
...
def get_size(path: str) -> str:
    try:
        ...
        logging.info(f"File size of {path}: {readable}")
        return readable
    except Exception as e:
        logging.error(f"Failed to get file size: {path} — {e}")
        raise
